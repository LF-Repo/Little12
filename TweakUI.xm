...
void loadPrefs() {
    @autoreleasepool {
        ...
        if (prefs) {
            ...
            NSDictionary *appSettings = [prefs objectForKey:mainIdentifier];
            if (appSettings) {
...
%ctor {
    @autoreleasepool {
        ...
        if (enabled) {
            BOOL isApp = [[[[NSProcessInfo processInfo] arguments] objectAtIndex:0] containsString:@"/Application"];
            ...
            if (isApp) {
                ...
                if (CFStringHasPrefix(bundleIdentifier, CFSTR("com.apple"))) {
                    ...
                    if (statusBarStyle == 2) {
                        %init(StatusBarX);
                        if (!wantsbottomInset) %init(bottominsetfix);
                    }
